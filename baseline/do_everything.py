...
def predict_whole_dir(
    predictor, test_dir, do_svs, output_json_path, onset_thres, offset_thres
):
    results = {}
    for song_dir in sorted(Path(test_dir).iterdir()):
        ...
        results = predict_one_song(
            ...
            tomidi=False,
            output_path=None,
            onset_thres=float(args.onset_thres),
            offset_thres=float(args.offset_thres),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--model_path", default="models/1005_e_4", help="model path"
    )
    parser.add_argument(
        "-s", "--svs", action="store_true", help="use Spleeter to extract vocal or not"
    )
    ...
    parser.add_argument(
        "-d", "--device", default="cuda:0", help="device to use if cuda is available"
    )
