...
def prepare_data(answer_true, answer_pred, time_shift):
    ...
    if time_shift >= 0.0:
        for i in range(len(answer_true)):
            if (
                answer_true[i] is not None
                and float(answer_true[i][1]) - float(answer_true[i][0]) > 0
        ...
        for i in range(len(answer_pred)):
            if (
                answer_pred[i] is not None
                and float(answer_pred[i][1]) - float(answer_pred[i][0]) > 0
...
def eval_one_data(
    answer_true, answer_pred, onset_tolerance=0.05, shifting=0, gt_pitch_shift=0
):
...
if __name__ == "__main__":
    for the_result in os.listdir(os.path.join(PROJECT_DIR, "results")):
        if the_result.startswith("predict"):
            evaluate(os.path.join(PROJECT_DIR, "results", the_result))
