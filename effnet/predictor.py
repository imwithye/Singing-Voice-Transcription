...
import numpy as np
from .net import EffNetb0, ResNet18, WideResidualNet
...
class EffNetPredictor:
    def __init__(self, device="cuda:0", model_path=None):
        ...
        if model_path is not None:
            ...
            self.model = WideResidualNet().to(self.device)
    ...
    def fit(self, train_dataset_dir, valid_dataset_dir, model_dir, **training_args):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if batch_idx % 10 == 0:
                    print(
                        ...
                        "loss:",
                        total_training_loss / (batch_idx + 1),
                    )
            ...
            if epoch % self.save_every_epoch == 0:
                ...
                target_model_path = Path(self.model_dir) / (
                    training_args["save_prefix"] + "_{}".format(epoch)
                )
                ...
                print(
                    "split train loss: onset {:.4f} offset {:.4f} pitch octave {:.4f} pitch class {:.4f}".format(
                ...
                print(
                    "split val loss: onset {:.4f} offset {:.4f} pitch octave {:.4f} pitch class {:.4f}".format(
        ...
        print(
            "Training done in {:.1f} minutes.".format((time.time() - start_time) / 60)
        )
    ...
    def _parse_frame_info(self, frame_info, onset_thres, offset_thres):
        ...
        for i in range(len(frame_info)):
            ...
            if info[0] >= onset_thres and onset_seq[i] == np.amax(
                onset_seq[backward_frames:forward_frames]
            ):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader)):
                ...
                onset_probs, offset_probs = (
                    torch.sigmoid(onset_logits).cpu(),
                    torch.sigmoid(offset_logits).cpu(),
                )
                pitch_octave_logits, pitch_class_logits = (
                    pitch_octave_logits.cpu(),
                    pitch_class_logits.cpu(),
                )
                ...
                for bid, song_id in enumerate(song_ids):
                    frame_info = (
                        onset_probs[bid],
                        offset_probs[bid],
                        torch.argmax(pitch_octave_logits[bid]).item(),
